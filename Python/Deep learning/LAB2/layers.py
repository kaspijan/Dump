#@author: for most parts FER, classes FC, ReLU, SoftmaxEntropyWithLogits and L2Regulizer finished by Simun Sopar
...
class FC(Layer):
    def __init__(self, input_layer, num_outputs, name,
        ...
        self.input_shape = input_layer.shape #oblika (N, veličina jednog podataka); npr. (N, H, W) za 2d slike
        self.N = self.input_shape[0] # broj podataka
        self.shape = (self.N, num_outputs)
        self.num_outputs = num_outputs

        self.num_inputs = 1 #broj stupnjeva slobode jednog podatka
        for i in range(1, len(self.input_shape)):
...
class ReLU(Layer):
    ...
    def forward(self, inputs):
        ...
        outputs = np.array([o if o > 0 else 0 for o in outputs])
    ...
    def backward_inputs(self, grads):
        ...
        Returns:
          ndarray of shape (N, C, H, W).
        """
        outputs = self.activations.flatten()
        tmp = grads.flatten()
        outputs = np.array([tmp[i] if outputs[i] > 0 else 0 for i in range(len(outputs))])
        outputs = outputs.reshape(self.activations.shape)
        return outputs
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        L1 = np.log(L1)
        L1 = np.sum(L1)
        #L2 = np.trace(np.dot(y.T, x))
        #L2 = np.trace(np.dot(x, y.T))
        L2 = np.sum(np.sum(x*y, axis = 1), axis = 0)
        
        return 1/x.shape[0] * (L1 - L2)
    ...
    def backward_inputs(self, x, y):
        ...
        for n, xn in enumerate(x):
            P[n] = softmax(xn)
            #tmp = softmax(xn)
            #P[n] = np.where(tmp == 0, 1e-10, tmp)
            #print(P[n])
            
        
        return 1/x.shape[0] * (P - y)
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        return self.weight_decay * np.dot(w, w)
    ...
    def backward_params(self):
        ...
        """
        grad_weights = 2 * self.weight_decay * self.weights.flatten()
